...
class MapViewController: VoltLinesViewController {
    ...
    var presenter: MapPresenter = { () in
            .init()
    }()
    
    let vwMap = MKMapView()
    lazy var btnList = ListButton()
    ...
    func initPresenter() {
        presenter.bookedTrip = bookedRoute
        presenter.didFetchedCoordinates = { list in
            self.setStations(list)
        }
        
        presenter.didSelectedLine = { line in
...
extension MapViewController {
    private func setLocation(_ location: CLLocation) {
        ...
        let userAnnotation = MyAnnotation(
            coordinate: CLLocationCoordinate2D(
                latitude: location.coordinate.latitude,
                longitude: location.coordinate.longitude),
            image: UIImage(named: Constants.kUserMarker))
        vwMap.addAnnotation(userAnnotation)
    }
    
    private func setStations(_ data: [StationMapModel]) {
        data.forEach { station in
            if let coordinate = station.coordinates {
                let coordinates = coordinate.components(separatedBy: Constants.kComma)
                ...
                let annotation = MyAnnotation(
                    ...
                    title: station.name ?? Constants.kEmpty,
                    subtitle: station.tripInfo,
                    coordinate: location,
                    image: station.booked == true ? UIImage(named: Constants.kBookedMarker) : UIImage(named: Constants.kDefaultMarker))
    ...
    private func routeDraw(_ lat: Double, long: Double) {
        ...
        let stationLoc = presenter.fetchSelectedStationCoords(bookedRoute)
