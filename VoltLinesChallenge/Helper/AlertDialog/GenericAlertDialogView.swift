...
class GenericAlertDialogView: UIView {
    ...
    lazy var lblTitle = createLabel(text: Constants.kAlertTitle, color: .mainBottomColor, font: UIFont(name: "Helvetica Neue", size: 16.0) ?? UIFont(), textAlign: .left, numberOfLines: 0)
    
    lazy var lblDes = createLabel(text: Constants.kAlertDesc, color: .mainBottomColor, font: UIFont(name: "Helvetica Neue", size: 16.0) ?? UIFont(), textAlign: .left, numberOfLines: 0)
    
    let btnBook: UIButton = {
        let btn = UIButton()
        btn.setTitle(Constants.kAlertBtn, for: .normal)
